...
저장소 루트에서 실행합니다. 아래 절차는 모델 없이 클러스터와 DNS를 확인합니다. 실제 추론은 다음 CPU 기반 추론 엔진 절차로 실행합니다.
...
`src/base`는 AIPerf를 통한 성능 측정을 지원하기 위해 `/v1/chat/completions` 엔드포인트를 제공합니다.
...
상세 내용은 [최소 요구사항](docs/requirements.md), [클러스터 사용](docs/local-k8s.md), [모델 관리](docs/models.md), [추론 API](docs/llama-api.md), [AIPerf 측정](docs/aiperf.md), [서비스 안정성 테스트](docs/availability-test.md), [CPU HPA 테스트](docs/hpa-test.md), [테스트 결과](docs/reports/README.md), [분석 리포트](docs/report.md)를 참고하세요.
