...
| `--concurrency` / `CONCURRENCIES` | `1,2,4,8` | 동시에 처리 중인 요청 수를 각 값으로 설정해 측정합니다. 자동 실행 스크립트는 동시성마다 별도 Job을 생성합니다. |
...
| `--no-server-metrics` | 활성화 | AIPerf의 서버 메트릭 수집을 끕니다. 실행 스크립트의 Kubernetes 자원 표본 수집은 별도입니다. |
| `--ui` | `none` | 대화형 UI 없이 실행합니다. |
| `--artifact-dir` | `/results/$(POD_NAME)` | Job의 결과 저장 경로입니다. 자동 실행 스크립트는 `/results/<run-id>/c<동시성>`으로 변경합니다. |

AIPerf 컨테이너는 CPU `1`, 메모리 `1Gi`를 requests와 limits에 동일하게 지정합니다.
