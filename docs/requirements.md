...
| 모델·토크나이저 다운로드 | `curl`, 체크섬 검증용 `sha256sum` 또는 `shasum` |
| 이미지 빌드 | `docker build` |
...
기본 [추론 Deployment](../k8s/llama-base/deployment.yaml)의 Pod 하나는 CPU 12코어와 메모리 16Gi를 요청합니다. 해당 Pod가 배치될 단일 노드에 요청량을 수용할 자원이 있어야 하며, Kubernetes와 함께 실행할 다른 워크로드용 여유 자원도 필요합니다. Docker Desktop을 사용하면 VM에 할당한 CPU·메모리도 확인합니다. 자원이 부족하면 Pod가 `Pending` 상태에 머물 수 있습니다. 측정용 자원을 조정할 때는 `requests`와 `limits`를 동일하게 유지합니다.
...
최초 설치, 이미지 빌드와 모델·토크나이저 다운로드에는 사용하는 파일의 배포처에 HTTPS로 접근할 수 있어야 합니다.
