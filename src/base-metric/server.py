...
from base.engine import LlamaEngine
from base.server import Settings, create_app as create_base_app, create_parser
...
class Metrics:
    def __init__(self):
        ...
        self.requests = Counter(
            "llama_requests_total", "Finished chat requests, including SSE errors.",
            ["status_code", "outcome"], registry=self.registry,
        )
        self.inflight = Gauge(
            "llama_requests_in_flight", "Chat requests including queued and streaming work.",
            registry=self.registry,
        )
        buckets = (0.01, 0.05, 0.1, 0.25, 0.5, 1, 2, 5, 10, 20, 30, 60, 120)
        self.duration = Histogram(
            "llama_request_duration_seconds", "Chat latency through the last response body.",
        ...
        self.tokens = Counter(
            "llama_tokens_total", "Tokens from completed engine generations.",
