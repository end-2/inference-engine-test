...
spec = importlib.util.spec_from_file_location("hpa_runner", ROOT / "scripts/hpa_test/runner.py")
        ...
        for error in (RuntimeError("scale-out timed out"), KeyboardInterrupt("Signal 2")):
            with self.subTest(error=error), tempfile.TemporaryDirectory() as directory, patch.object(runner, "ROOT", Path(directory)):
                ...
                with self.assertRaises(type(error)):
                    e.execute()
                e.command.assert_any_call("-n", "hpa-test", "scale", "deployment/aiperf", "--replicas=0")
                e.command.assert_any_call("-n", "hpa-test", "wait", "--for=delete", "pod", "-l", "app=aiperf", "--timeout=150s", timeout=170)
