...
"$KUBECTL" kustomize "$ROOT/k8s/hpa-test" > "$rendered/hpa.yaml"
...
if grep -Fq 'availability-test' "$rendered/hpa.yaml"; then fail 'Stale availability namespace or RBAC'; fi
grep -Fq 'name: hpa-test-node-metrics' "$rendered/hpa.yaml" || fail 'Missing isolated node RBAC'
...
awk 'BEGIN { RS="---" } /kind: Deployment/ && /name: llama-base-metric/ { print }' "$rendered/hpa.yaml" > "$rendered/engine.yaml"
...
printf 'PASS: HPA, namespace isolation, monitoring, load configuration, and resource guarantees\n'
