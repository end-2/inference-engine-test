...
class NativeEnhancedTests(unittest.TestCase):
    ...
    def test_native_batch_concurrent_requests_and_stream_tokens(self):
        engine = self.engine("enhanced-batch-llamacpp", max_parallel=3, prefill_chunk=8, n_ubatch=64)
    ...
    def test_native_cache_spill_promotion_and_restart_match_uncached_output(self):
        ...
        engine = self.engine("enhanced-cache-llamacpp", **options)
        ...
        engine.close()
        restarted = self.engine("enhanced-cache-llamacpp", **options)
    ...
    def test_restored_partial_prefix_matches_cold_inference(self):
        engine = self.engine("enhanced-cache-llamacpp", cache_dir=Path(self.directory.name),
        ...
        cold = self.engine("base-llamacpp")
