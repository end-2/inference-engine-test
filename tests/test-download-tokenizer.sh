...
cp "$ROOT/scripts/download-tokenizer.sh" "$sandbox/scripts/"
export TOKENIZER_FIXTURES="$sandbox/fixtures" CURL_TRACE="$sandbox/curl-trace"
checksums="$sandbox/config/models/qwen2.5-0.5b-tokenizer.sha256"
...
cat > "$sandbox/config/models/qwen2.5-0.5b-tokenizer.env" <<'EOF_CONFIG'
...
script="$sandbox/scripts/download-tokenizer.sh"
