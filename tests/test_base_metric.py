...
import test_llama_server as base_tests

spec = importlib.util.spec_from_file_location(
    "base_metric_server", Path(__file__).resolve().parents[1] / "src/base-metric/server.py",
)
...
class MetricServerTests(base_tests.ServerTests):
    async def asyncSetUp(self):
        with patch("test_llama_server.create_app", metric_server.create_app):
    ...
    async def test_metrics_exclude_probes_and_count_completed_tokens(self):
        ...
        self.assertEqual(self.value("llama_requests_total", status_code="200", outcome="success"), 2)
        self.assertEqual(self.value("llama_tokens_total", kind="prompt"), 16)
        self.assertEqual(self.value("llama_tokens_total", kind="completion"), 6)
        self.assertEqual(self.value("llama_request_duration_seconds_count", outcome="success"), 2)
        self.assertEqual(self.value("llama_time_to_first_token_seconds_count"), 1)
        self.assertEqual(self.value("llama_requests_in_flight"), 0)
    ...
    async def test_http_and_sse_errors_have_distinct_outcomes(self):
        ...
        for status in ("404", "500", "200"):
            self.assertEqual(self.value("llama_requests_total", status_code=status, outcome="error"), 1)
        self.assertIsNone(self.value("llama_requests_total", status_code="200", outcome="success"))
        self.assertEqual(self.value("llama_tokens_total", kind="completion"), 0)
        self.assertEqual(self.value("llama_time_to_first_token_seconds_count"), 0)
    ...
    async def test_metrics_remain_responsive_while_generation_is_blocked(self):
        ...
        try:
            ...
            self.assertEqual(self.value("llama_requests_in_flight"), 1)
            self.assertEqual(self.value("llama_time_to_first_token_seconds_count"), 0)
        finally:
    ...
    async def test_disconnected_stream_is_counted(self):
        ...
        self.assertEqual(self.value("llama_requests_total", status_code="200", outcome="disconnected"), 1)
        self.assertEqual(self.value("llama_requests_in_flight"), 0)
    ...
    async def test_app_registries_are_isolated(self):
        ...
        self.assertEqual(registry.get_sample_value("llama_tokens_total", {"kind": "prompt"}), 0)
        self.assertIsNone(registry.get_sample_value(
            "llama_requests_total", {"status_code": "200", "outcome": "success"},
        ))
