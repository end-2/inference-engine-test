...
spec = importlib.util.spec_from_file_location(
    "base_metric_server", Path(__file__).resolve().parents[1] / "src/llamacpp/base_metric/server.py",
)
...
class MetricServerTests(base_tests.ServerTests):
    async def asyncSetUp(self):
        with patch("test_server_llamacpp.create_app", metric_server.create_app):
