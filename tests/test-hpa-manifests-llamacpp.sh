...
"$KUBECTL" kustomize "$ROOT/k8s/hpa-test-llamacpp" > "$rendered/hpa.yaml"
...
grep -Fq 'name: hpa-node-metrics-llamacpp' "$rendered/hpa.yaml" || fail 'Missing isolated node RBAC'
...
awk 'BEGIN { RS="---" } /kind: Deployment/ && /name: base-metric-llamacpp/ { print }' "$rendered/hpa.yaml" > "$rendered/engine.yaml"
