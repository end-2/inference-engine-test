...
HELPERS = ROOT / "scripts/availability_test_transformers"
...
class Experiment:
    def __init__(self, args):
        ...
        self.ns = ["-n", "availability-test-transformers"]
    ...
    def reset_inference(self):
        self.scale("transformers-base-metric", 0)
        self.kubectl(
            *self.ns,
            "wait",
            "--for=delete",
        ...
        self.kubectl(
            *self.ns,
            "patch",
            "deployment/transformers-base-metric",
            "--type=merge",
            "-p",
            json.dumps(patch),
        )
        self.scale("transformers-base-metric", 2)
        self.kubectl(
            *self.ns,
            "rollout",
            "status",
        ...
        pods = self.get(
            self.ns + ["get", "pods", "-l", "app=transformers-base-metric", "-o", "json"]
        )["items"]
    ...
    def measure(self):
        ...
        if self.mode == "sigkill":
            source = f"{self.victim}:/var/log/pods/availability-test-transformers_{self.s['victim_pod']}_{self.s['victim_uid']}/api/0.log"
    ...
    def execute(self):
        ...
        self.s = dict(
            ...
            raw=str(self.raw),
            kube=self.k,
            namespace="availability-test-transformers",
...
def main():
    ...
    parser.add_argument(
        "--cluster", default=os.environ.get("CLUSTER_NAME", "transformers-tests")
    )
    ...
    with (lockdir / "transformers-multinode.lock").open("a") as lock:
