...
NAMESPACE = "hpa-test-transformers"
ENGINE = "transformers-base-metric"
...
class Experiment:
    ...
    def preflight(self):
        ...
        for label, count in (("engine", 2), ("monitor", 1)):
            workers = [n for n in nodes if n["metadata"].get("labels", {}).get("workload") == label]
            if len(workers) != count or any(
                "node-role.kubernetes.io/control-plane" in n["metadata"].get("labels", {})
                for n in workers
            ):
                raise RuntimeError("Requires exactly two engine workers and one monitor worker")
        ...
        if target != {"apiVersion": "apps/v1", "kind": "Deployment", "name": ENGINE} or (
            ...
            raise RuntimeError("Expected one CPU utilization metric targeting transformers-base-metric")
    ...
    def execute(self):
        ...
        self.raw = ROOT / "reports" / "transformers" / identifier
...
def parse_args(scenario="scale-out-in", argv=None):
    selector = argparse.ArgumentParser(add_help=False)
    selector.add_argument("--scenario", choices=("scale-out", "scale-out-in"), default=scenario)
    scenario = selector.parse_known_args(argv)[0].scenario
    descriptions = {
    ...
    parser = argparse.ArgumentParser(description=descriptions[scenario], parents=[selector])
    parser.set_defaults(scenario=scenario, low_request_rate=None)
    parser.add_argument("--cluster", default=os.environ.get("CLUSTER_NAME", "transformers-tests"))
...
def main(scenario="scale-out-in"):
    ...
    with (Path(kubeconfig).parent / "transformers-multinode.lock").open("a") as lock:
