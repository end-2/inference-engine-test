...
NAMESPACE = "hpa-test-llamacpp"
ENGINE = "base-metric-llamacpp"
...
class Experiment:
    ...
    def preflight(self):
        ...
        if target != {"apiVersion": "apps/v1", "kind": "Deployment", "name": ENGINE} or (
            ...
            raise RuntimeError("Expected one CPU utilization metric targeting base-metric-llamacpp")
    ...
    def collect(self):
        ...
        dashboard = json.loads((ROOT / "k8s/hpa-test-llamacpp/grafana/hpa.json").read_text())
    ...
    def execute(self):
        ...
        self.raw = ROOT / "reports" / "llamacpp" / identifier
...
def main(scenario="scale-out-in"):
    ...
    with (Path(kubeconfig).parent / "hpa-test-llamacpp.lock").open("a") as lock:
