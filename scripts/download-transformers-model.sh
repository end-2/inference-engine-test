#!/bin/sh
# Verify the pinned weights and tokenizer before publishing the model directory.
set -eu
umask 022
...
target="$models_dir/$MODEL_DIRECTORY"
...
while read -r expected file; do
    ...
    if ! verify "$staging/$file.part" "$expected"; then
        ...
        die "Downloaded model checksum mismatch: $file. Rerun the download."
    fi
