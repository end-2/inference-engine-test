...
HELPERS = ROOT / "scripts/availability_test_llamacpp"
...
class Experiment:
    def __init__(self, args):
        ...
        self.ns = ["-n", "availability-test-llamacpp"]
    ...
    def reset_inference(self):
        self.scale("base-metric-llamacpp", 0)
        self.kubectl(
            *self.ns,
            "wait",
            "--for=delete",
        ...
        self.kubectl(
            *self.ns,
            "patch",
            "deployment/base-metric-llamacpp",
            "--type=merge",
            "-p",
            json.dumps(patch),
        )
        self.scale("base-metric-llamacpp", 2)
        self.kubectl(
            *self.ns,
            "rollout",
            "status",
        ...
        pods = self.get(
            self.ns + ["get", "pods", "-l", "app=base-metric-llamacpp", "-o", "json"]
        )["items"]
    ...
    def measure(self):
        ...
        if self.mode == "sigkill":
            source = f"{self.victim}:/var/log/pods/availability-test-llamacpp_{self.s['victim_pod']}_{self.s['victim_uid']}/api/0.log"
    ...
    def execute(self):
        ...
        self.s = dict(
            ...
            raw=str(self.raw),
            kube=self.k,
            namespace="availability-test-llamacpp",
...
def main():
    ...
    with (lockdir / "availability-test-llamacpp.lock").open("a") as lock:
