...
select_provider() {
    ...
    case ${KIND_EXPERIMENTAL_PROVIDER:-docker} in
        ...
        *) die "This setup uses Docker. KIND_EXPERIMENTAL_PROVIDER must be docker or auto." ;;
    esac
...
test_smoke() {
    job="kind-smoke-test"
    kube delete job "$job" --ignore-not-found=true
    kube create job "$job" --image=docker.io/library/busybox:1.37.0 -- \
        sh -c 'echo smoke-ok; nslookup kubernetes.default.svc.cluster.local'
    kube wait --for=condition=Complete "job.batch/$job" --timeout="$WAIT_TIMEOUT"
    kube logs "job.batch/$job"
    kube delete job "$job" --ignore-not-found=true
}
